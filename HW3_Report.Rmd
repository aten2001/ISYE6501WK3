...
[^1]: It is my belief that the order of transformation is correlation removal -> Box-Cox -> center/scale (either one can go first). The key here is that Box-Cox **must** be carried out before centering and scaling because it involves the possibility of raising the predictors to negative powers. If you center and scale first you can run into division by 0 even if the predictor doesn't originally contain 0. If the predictor already has 0 you cannot use Box-Cox transformation.
...
With the exception of `M.F` the improvement to the predictor distribution is quite remarkable. This set of transformations shall be used with the addition of the categorical predictor `So` prior to automatic feature selection.
...
# Include So in processed predictors.
data.processed <- data.frame(data.processed, So = data.predictors$So)
...
stopCluster(cl)
# for (comb_ind in 1:length(var_combo)) {
#     formula = as.formula(paste('Crime ~ ', paste(varnames[var_combo[comb_ind,]], collapse = ' + '), sep = ''))
#     
#     
#     combo_mse <- rep(0, 10)
#     for (fold_ind in 1:10) {
#         training <- data.training[-folds[[fold_ind]],]
#         validation <- data.training[folds[[fold_ind]],]
#         
#         model <- lm(formula, data = training)
#         
#         pred <- predict(model, validation[, !(names(validation) %in% c('Crime'))])
#         
#         combo_mse[fold_ind] <- mse(validation$Crime, pred)
#     }
#     
#     mse_est[comb_ind] <- mean(combo_mse)
# }

min_ind <- which.min(mse_est)

mse_est[min_ind]
names(data.training)[var_combo[min_ind,]]

```

The preprocessed data is now split into a test (~10%) & training set (~90%). With 14 predictors (13 preprocessed numerical predictors + `So`), the number of predictor combinations is `r nrow(var_combo) - 1` (the trivial combination where no predictor is selected is not included). An exhaustive search over the entire combination space is carried out. 10-fold cross-validation on the training data with the average of MSE from the model prediction on the validation fold is used to estimate out-of-sample accuracy. The predictor combination that results in the the lowest out-of-sample accuracy estimate is selected.
