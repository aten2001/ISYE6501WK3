...
```{r atlanta, fig.cap = 'Summer Temperature in Atlanta from 1996 to 2015'}
plot(data.ts, title = 'Atlanta Summer Temperature (1996 - 2015)', xlab = 'Days Since June 30', ylab = 'Temperature (F)')
```
...
Figure \@ref(fig:trendseason) shows the component plot for a series with both trend and season. Once again. Trend is picked up by the moving average level. Seasonality is correctly decomposed however. The smoothing parameters are $\alpha$ = $`r persistence2[1]`$, $\beta$ = $`r persistence2[2]`$, $\gamma$ = $`r persistence2[3]`$.
...
Since there isn't any discernable ampltitude change in Figure \@ref(ref:atlanta), an additive model is chosen for the seasonality component. An AAA triple smoothing model is constructed on the actual temperature data. The smoothing parameters are $\alpha$ = $`r persistence[1]`$, $\beta$ = $`r persistence[2]`$, $\gamma$ = $`r persistence[3]`$. Figure \@(ref:tempscomp) shows the time series components. 

```{r tempscomp, fig.cap = 'Time series components of Atlanta Summer Temperatures (1996 - 2015)'}
plot(data.es$states, main = 'Time Series Components')
```

Interestingly there is a bit of negative trend in the beginning. However it quickly goes back to roughly 0 and does not exhibit the characteristic "bumps" as in the case with simulated period inflation data. The elephant in the room, the level component of the time series looks quite noisy and perhaps can be argued to exhibit some sort of cycles. However it does not show a relatively straight center as expected if the seasonality is indeed using the average period of the time series and the period of the level cycles seems longer than the seasonality. Comparing to the effects shown using simulated data, the Atlanta summer periods have not become longer and there is no clear long term increase in Atlanta summer temperatures, either.
